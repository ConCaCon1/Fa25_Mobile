  ...
  const fetchReports = async () => {
    try {
      const res = await apiGet(
        `/ships/${shipId}/report-problems?page=1&size=50&sort=createdDate,desc`
      );
      if (res?.data?.items) {
  ...
  const renderItem = ({ item }) => {
    ...
    return (
      <TouchableOpacity
        style={styles.card}
        onPress={() =>
          navigation.navigate("ReportProblemDetail", { id: item.id })
        }
      >
        <View
          style={[styles.statusStrip, { backgroundColor: statusInfo.color }]}
        />
          ...
          ListEmptyComponent={
            <View style={styles.emptyContainer}>
              ...
              <Text style={styles.emptyText}>
                Tàu này chưa có báo cáo sự cố nào.
              </Text>
